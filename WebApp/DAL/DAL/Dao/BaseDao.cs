...
{
    public class BaseDao : WmsContext, IDisposable
    {
        public BaseDao()
        ...
        public int Saves(string functiongGr, string action, object dto, string tableName)
        {
            ...
            try
            {
                ...
                //Save history
                //SaveHitory(functiongGr, action, dto, tableName);
            }
            catch (Exception ex)
            {
                returnCode = ProcessDbException(ex);
        ...
        /// <param name="tableName"></param>
        /// <returns></returns>
        private int SaveHitory(string functiongGr, string action, object dto, string tableName)
        {
            // initial sys_history object
            // the flag isList = true : list
            var isList = false;
            ...
            if (dto != null)
            {
                var currentType = dto.GetType();
                var props = new List<PropertyInfo[]>();

                // case object is list
                if (currentType.Name.Equals("List`1"))
                {
                    var objs = ((IEnumerable)dto).Cast<object>().ToList();
                    foreach (var obj in objs)
                    {
                        // assign the value of isList : true
                        isList = true;
                ...
                {
                    // assign the value of isList : false
                    isList = false;
                ...
                foreach (var _prop in props)
                {
                    ...
                    hisDTO.EmployeeName = UserSession.UserName;
                    //hisDTO.EmployeeName = ;
                    ...
                    foreach (var prop in _prop)
                    {
                        ...
                        if (isList)
                        {
                            if (prop.GetValue(((IEnumerable)dto).Cast<object>().ToList()[indexList], null) != null)
                                value = prop.GetValue(
                                    (
                                        (IEnumerable)dto).Cast<object>().ToList()[indexList], null
                                    ).ToString();
                        }
                        else
                        {
                            if (prop.GetValue(dto, null) != null)
                                value = prop.GetValue(dto, null).ToString();
                        }
                        ...
                        if (!name.Equals(string.Empty))
                        {
                            ...
                            switch (numRow)
                            {
                                ...
                                #endregion "Set value for sys_historydetail object"
                            }
                        }
                    }
