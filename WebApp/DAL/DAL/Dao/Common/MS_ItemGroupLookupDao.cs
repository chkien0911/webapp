...
{
    public class MS_ItemGroupLookupDao : BaseDao
    {
        /// <summary>
        /// Search Data for Combobox in ms_ItemGroup table
        /// </summary>
        ...
        public int SelectData(out DataTable dtResult)
        {
            ...
            using (BaseDao context = new BaseDao())
            {
                try
                {
                    IQueryable<ms_itemgrp> lstItemGroup = context.ms_itemgrp.AsQueryable();

                    #region Search data

                    IEnumerable<MS_ItemGroupModel> lstResult = lstItemGroup.Select(ig => new MS_ItemGroupModel
                    {
                        Code = ig.Code,
                        Name1 = ig.Name1,
                        Name2 = ig.Name2,
        ...
        public int SelectData<T>(out List<T> lstResult)
        {
            ...
            using (BaseDao context = new BaseDao())
            {
                try
                {
                    IQueryable<ms_itemgrp> query = context.ms_itemgrp.AsQueryable();

                    #region Search data

                    lstResult = query.Select(ig => new MS_ItemGroupModel
                    {
                        ID = ig.ID,
                        Code = ig.Code,
                        Name1 = ig.Name1,
